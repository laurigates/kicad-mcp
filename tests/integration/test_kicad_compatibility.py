...
class TestKiCadCompatibility:
    ...
    async def test_project_creation_format(self, temp_project_dir, mock_context):
        ...
        assert sch_content.startswith("(kicad_sch"), (
            f"Schematic doesn't start with (kicad_sch: {sch_content[:50]}"
        )
        ...
        assert pcb_content.startswith("(kicad_pcb"), (
            f"PCB doesn't start with (kicad_pcb: {pcb_content[:50]}"
        )
    ...
    def test_schematic_component_positioning(self, temp_project_dir):
        ...
        for i, pos1 in enumerate(positions):
            for pos2 in positions[i + 1 :]:
                ...
                assert distance >= 10, (
                    f"Components too close: {pos1} and {pos2} (distance: {distance}mm)"
                )
...
"""
        ...
        assert len(circuit.power_symbols) == 2, (
            f"Expected 2 power symbols, got {len(circuit.power_symbols)}"
        )
        assert len(circuit.connections) == 3, (
            f"Expected 3 connections, got {len(circuit.connections)}"
        )
