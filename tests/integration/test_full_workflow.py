...
class TestFullWorkflow:
    ...
    async def test_complete_project_workflow(self, mcp_server, mock_context, temp_dir):
        ...
        assert result["success"] is True
        esp32_uuid = result["component_uuid"]
    ...
    @pytest.mark.asyncio
    async def test_project_discovery_and_analysis(self, mcp_server, mock_context, sample_kicad_project):
        ...
        result = await list_projects_func(
            search_directories=[str(project_dir)],
            ctx=mock_context
        )
        assert result["success"] is True
    ...
    async def test_large_project_workflow(self, mcp_server, mock_context, temp_dir):
        ...
        print(f"Performance: {component_add_time:.2f}s for components, "
    ...
    async def test_concurrent_operations(self, mcp_server, mock_context, temp_dir):
        ...
        async def extract_netlist(project_path, project_name):
            return await extract_netlist_func(
                schematic_path=f"{project_path}/{project_name}.kicad_sch",
                ctx=mock_context
            )
