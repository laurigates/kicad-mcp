...
class TestCircuitTools:
    ...
    async def test_add_component_to_circuit_esp32(self, mock_context, sample_kicad_project):
        ...
        if schematic_content.strip().startswith('{'):
            ...
            esp32_component = next((comp for comp in components 
                                  if comp.get("reference") == "U1"), None)
    ...
    async def test_add_power_symbols(self, mock_context, sample_kicad_project):
        ...
        if schematic_content.strip().startswith('{'):
            ...
            power_components = [comp for comp in components 
    ...
    async def test_component_positioning(self, mock_context, sample_kicad_project):
        ...
        if schematic_content.strip().startswith('{'):
            ...
            c1_component = next((comp for comp in components 
                               if comp.get("reference") == "C1"), None)
    ...
    async def test_component_reference_uniqueness(self, mock_context, sample_kicad_project):
        ...
        assert result1["reference"] != result2["reference"] or result1["component_uuid"] != result2["component_uuid"]
    ...
    async def test_complex_circuit_creation(self, mock_context, temp_dir):
        ...
        # Add components
        components = [
            {"type": "MCU_Espressif:ESP32-WROOM-32", "ref": "U1", "value": "ESP32", "x": 1000, "y": 1000},
        ...
        assert result["success"] is True
        assert result["validation_results"]["component_count"] >= 5  # 3 components + 2 power symbols