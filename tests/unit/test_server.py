...
from kicad_mcp.server import cleanup_handler, main, setup_signal_handlers
...
class TestServerLifecycle:
    ...
    def test_cleanup_handler_basic(self):
        ...
        with patch("kicad_mcp.server.logger") as mock_logger:
            ...
            cleanup_calls = [
                call for call in mock_logger.info.call_args_list if "cleanup" in str(call).lower()
            ]
    ...
    def test_cleanup_handler_exception_handling(self, mock_logger):
        ...
        with patch("kicad_mcp.server.cleanup_temp_dirs", side_effect=Exception("Cleanup error")):
            ...
            error_calls = [
                call for call in mock_logger.error.call_args_list if "cleanup" in str(call).lower()
            ]
    ...
    async def test_main_server_startup(self):
        ...
        with (
            patch("kicad_mcp.server.FastMCP") as mock_fastmcp,
            patch("kicad_mcp.server.setup_signal_handlers") as mock_signals,
            patch("kicad_mcp.server.logger"),
        ):
            ...
            with (
                patch("kicad_mcp.server.register_project_tools") as mock_reg_project,
                patch("kicad_mcp.server.register_netlist_tools") as mock_reg_netlist,
                patch("kicad_mcp.server.register_export_tools") as mock_reg_export,
                patch("kicad_mcp.server.register_drc_tools") as mock_reg_drc,
                patch("kicad_mcp.server.register_bom_tools") as mock_reg_bom,
                patch("kicad_mcp.server.register_pattern_tools") as mock_reg_pattern,
                patch("kicad_mcp.server.register_analysis_tools") as mock_reg_analysis,
                patch("kicad_mcp.server.register_circuit_tools") as mock_reg_circuit,
            ):
                # Test main function
                await main()
    ...
    async def test_main_exception_handling(self):
        ...
        with (
            patch("kicad_mcp.server.FastMCP", side_effect=Exception("Server startup failed")),
            patch("kicad_mcp.server.logger") as mock_logger,
            patch("kicad_mcp.server.cleanup_handler") as mock_cleanup,
        ):
            # Should not raise exception
            await main()
            ...
            # Should log error
            error_calls = [
                call for call in mock_logger.error.call_args_list if "error" in str(call).lower()
            ]
            assert len(error_calls) > 0
    ...
    def test_server_configuration(self, mock_fastmcp):
        ...
        with (
            patch("kicad_mcp.server.setup_signal_handlers"),
            patch("kicad_mcp.server.logger"),
            patch("kicad_mcp.server.register_project_tools") as mock_reg,
        ):
    ...
    def test_cleanup_handler_multiple_calls(self):
        ...
        with (
            patch("kicad_mcp.server.logger") as mock_logger,
            patch("kicad_mcp.server.cleanup_temp_dirs") as mock_cleanup_temp,
        ):
    ...
    def test_logger_configuration(self):
        ...
        with patch("kicad_mcp.server.setup_logging"):
    ...
    async def test_server_shutdown_graceful(self):
        ...
        with patch("kicad_mcp.server.FastMCP") as mock_fastmcp:
            ...
            with (
                ...
                patch("kicad_mcp.server.logger") as mock_logger,
                patch("kicad_mcp.server.cleanup_handler") as mock_cleanup,
                patch("kicad_mcp.server.register_project_tools"),
                patch("kicad_mcp.server.register_netlist_tools"),
                patch("kicad_mcp.server.register_export_tools"),
                patch("kicad_mcp.server.register_drc_tools"),
                patch("kicad_mcp.server.register_bom_tools"),
                patch("kicad_mcp.server.register_pattern_tools"),
                patch("kicad_mcp.server.register_analysis_tools"),
                patch("kicad_mcp.server.register_circuit_tools"),
            ):
    ...
    def test_pid_logging(self):
        ...
        with (
            ...
            [call for call in mock_logger.info.call_args_list if "12345" in str(call)]
    ...
    async def test_concurrent_cleanup_calls(self):
        ...
        with (
            ...
            await asyncio.gather(call_cleanup(), call_cleanup(), call_cleanup())
