...
class TextToSchematicParser:
    ...
    def parse_yaml_circuit(self, yaml_text: str) -> Circuit:
        ...
        try:
            ...
            raise ValueError(f"Error parsing YAML circuit: {str(e)}")
    ...
    def _parse_component(self, comp_desc: str) -> Optional[Component]:
    ...
    def _parse_component_simple(self, line: str) -> Optional[Component]:
    ...
    def _parse_power_symbol(self, power_desc: str) -> Optional[PowerSymbol]:
    ...
    def _parse_power_symbol_simple(self, line: str) -> Optional[PowerSymbol]:
    ...
    def _parse_connection(self, conn_desc: str) -> Optional[Connection]:
    ...
    def _parse_connection_simple(self, line: str) -> Optional[Connection]:
    ...
    async def create_circuit_from_text(
        ...
        try:
            ...
            from kicad_mcp.tools.circuit_tools import (
                ...
                create_wire_connection as _create_wire_connection
            )
            ...
            for i, component in enumerate(circuit.components):
                try:
                    ...
                    if result["success"]:
                        ...
                        results["errors"].append(f"Failed to add {component.reference}: {result.get('error', 'Unknown error')}")
                
                except Exception as e:
                    results["errors"].append(f"Error adding component {component.reference}: {str(e)}")
            ...
            for power_symbol in circuit.power_symbols:
                try:
                    ...
                    if result["success"]:
                        ...
                        results["errors"].append(f"Failed to add power symbol {power_symbol.power_type}: {result.get('error', 'Unknown error')}")
                
                except Exception as e:
                    results["errors"].append(f"Error adding power symbol {power_symbol.power_type}: {str(e)}")
            ...
            for i, connection in enumerate(circuit.connections):
                try:
                    ...
                    results["connections_created"].append(f"{connection.start_component} -> {connection.end_component}")
                except Exception as e:
                    results["errors"].append(f"Error creating connection: {str(e)}")
            
            if ctx:
                await ctx.report_progress(100, 100)
                await ctx.info(f"Circuit creation complete: {len(results['components_added'])} components, {len(results['power_symbols_added'])} power symbols")
    ...
    async def validate_circuit_description(
        ...
        try:
            ...
            validation_results = {
                ...
                "components": [{"ref": c.reference, "type": c.component_type, "value": c.value} for c in circuit.components],
                "power_symbols": [{"ref": p.reference, "type": p.power_type} for p in circuit.power_symbols],
                "connections": [f"{c.start_component} -> {c.end_component}" for c in circuit.connections],
                "warnings": []
            }
            ...
            if ctx:
                await ctx.info(f"Validation complete: {len(circuit.components)} components, {len(circuit.connections)} connections")
