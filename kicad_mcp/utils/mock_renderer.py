...
class MockSchematicRenderer:
    ...
    def parse_schematic_components(self, schematic_file: str) -> dict[str, Any]:
        ...
        for match in re.finditer(wire_pattern, content):
            ...
            wires.append({"start": (float(x1), float(y1)), "end": (float(x2), float(y2))})
...
'''
        ...
        # Add title
        svg_content += (
            f'  <text x="{width / 2}" y="20" class="title">{schematic_data["title"]}</text>\n\n'
        )
    ...
    def _render_component(
        ...
        if comp_type == "resistor":
            ...
            svg_content += "  <g>\n"
            svg_content += (
                f'    <rect x="{x - 30}" y="{y - 10}" width="60" height="20" class="component"/>\n'
            )
            ...
            svg_content += "  <g>\n"
            svg_content += (
                f'    <rect x="{x - 25}" y="{y - 15}" width="50" height="30" class="component"/>\n'
            )
            ...
            svg_content += "  <g>\n"
            svg_content += (
                f'    <rect x="{x - 40}" y="{y - 20}" width="80" height="40" class="component"/>\n'
            )
            svg_content += f'    <text x="{x}" y="{y - 25}" class="text">{reference}</text>\n'
            svg_content += (
                f'    <text x="{x}" y="{y + 5}" class="text" font-size="10">{value}</text>\n'
            )
            svg_content += "  </g>\n"
            ...
            svg_content += "  <g>\n"
            svg_content += (
                f'    <rect x="{x - 20}" y="{y - 10}" width="40" height="20" class="component"/>\n'
            )
            svg_content += f'    <text x="{x}" y="{y - 15}" class="text">{reference}</text>\n'
            svg_content += f'    <text x="{x}" y="{y + 25}" class="text">{value}</text>\n'
            svg_content += "  </g>\n"
